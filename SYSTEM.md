...
tables:
  owns:
    ...
    - fab_time_entries
    - fab_weekly_entries
    - job_bom                 # fab owns it; purchasing READS it (Lane 7, 07/09/2026)
  reads:
    - jobs
    - profiles
    - job_bom
...
4. **What it borrows.** It reads the shared `jobs` table and `profiles` directly,
   ...
   fails `npm run test:architecture`. `job_bom` is fab's own table (the BOM
   import writes it); purchasing READS it.
